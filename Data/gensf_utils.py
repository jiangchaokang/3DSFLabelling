...
def get_rigid_transform(R, t):
    ...
    if len(t.shape)==1:
        t = t.unsqueeze(0)
    transform = transforms.Transform3d(device='cpu').rotate(R).translate(t)
    return transform
...
def box_coordinate(points, boxes, inverse=False):
    ...
    transform = transforms.Transform3d(device='cpu').translate(-centers).rotate(rot_matrix.transpose(-1,-2))
    if inverse:
...
def inside_box(points, boxes, padded=True):
    ...
    box_coord = points.cpu().update_padded(box_coordinate(points.points_padded().cpu(), boxes.cpu()))
...
def box2corners(box_params):
    ...
    forward = torch.stack([-torch.sin(box_params[:,7]), torch.cos(box_params[:,7]), torch.zeros(box_params.shape[0], device='cpu', dtype=torch.float32)], dim=-1) * box_params[:,5:6] / 2
    right = torch.stack([torch.cos(box_params[:,7]), torch.sin(box_params[:,7]), torch.zeros(box_params.shape[0], device='cpu', dtype=torch.float32)], dim=-1) * box_params[:,4:5] / 2
    up = torch.stack([torch.zeros(box_params.shape[0], device='cpu', dtype=torch.float32), torch.zeros(box_params.shape[0], device='cpu', dtype=torch.float32), box_params[:,6] / 2], dim=-1)
...
def tighten_boxes(boxes, points):
    ...
    points_r, boxes =  points_r.cpu(), boxes.cpu()
    ...
    box_coord_pc = box_coord_pc.cpu()
    box_shape = box_shape.cpu()
    ...
    tightened_centers, boxes = tightened_centers.cpu(), boxes.cpu()
    ...
    tightened_shape = tightened_shape.cpu()
