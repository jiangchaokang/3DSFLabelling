...
class SceneFlowDataset(Dataset):
    ...
    def subsample_points(self, sequence, ground_truth):
        ...
        if sequence[0].shape[0] < self.nb_points:
            num_copies = self.nb_points // sequence[0].shape[0]
            remainder = self.nb_points % sequence[0].shape[0]
            sequence[0] = np.tile(sequence[0], (num_copies, 1))
            sequence[1] = np.tile(sequence[1], (num_copies, 1))
            ground_truth = [np.tile(g, (num_copies, 1)) for g in ground_truth]
            
            if remainder > 0:
                ind1 = np.random.permutation(sequence[0].shape[0])[:remainder]
                sequence[0] = np.concatenate((sequence[0], sequence[0][ind1]), axis=0)
                sequence[1] = np.concatenate((sequence[1], sequence[1][ind1]), axis=0)
                ground_truth = [np.concatenate((g, g[ind1]), axis=0) for g in ground_truth]
        else:
            ind1 = np.random.permutation(sequence[0].shape[0])[: self.nb_points]
            sequence[0] = sequence[0][ind1]
            sequence[1] = sequence[1][ind1]
            ground_truth = [g[ind1] for g in ground_truth]

        # Choose point in second scan  TODO
        # if sequence[1].shape[0] < self.nb_points:
        #     sequence[1] = np.random.choice(sequence[1], size=self.nb_points, replace=True, p=None)
        # else:
        #     sequence[1] = sequence[1][ind1]
        # if sequence[1].shape[0] < self.nb_points:
        #     num_copies = self.nb_points // sequence[1].shape[0]
        #     remainder = self.nb_points % sequence[1].shape[0]
        #     sequence[1] = np.tile(sequence[1], (num_copies, 1))
        #     if remainder > 0:
        #         ind2 = np.random.permutation(sequence[1].shape[0])[:remainder]
        #         sequence[1] = np.concatenate((sequence[1], sequence[1][ind2]), axis=0)
        # else:
